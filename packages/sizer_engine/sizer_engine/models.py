...
class TradeType(str, Enum):
    auto="auto"; lottery="lottery"; premium="premium"; position="position"; shortterm="shortterm"; prediction="prediction"; trading="trading"
class EdgeSource(str, Enum):
    exact_math="exact_math"; live_track_record="live_track_record"; backtest="backtest"; related_experience="related_experience"; guess="guess"
class PayoffStructure(str, Enum):
    binary="binary"; continuous="continuous"; capped="capped"; unbounded="unbounded"
class CapacityPolicy(str, Enum):
    reject="reject"; downsize="downsize"; queue="queue"
class TailProfile(str, Enum):
    normal="normal"; moderate="moderate"; heavy="heavy"; extreme="extreme"
class FieldConfidence(str, Enum):
    user_stated="user_stated"; inferred="inferred"; guessed="guessed"

def _clean(obj):
    if isinstance(obj, Enum): return obj.value
    if is_dataclass(obj): return {k:_clean(v) for k,v in asdict(obj).items()}
    if isinstance(obj, dict): return {k:_clean(v) for k,v in obj.items()}
    if isinstance(obj, (list, tuple)): return [_clean(v) for v in obj]
    return obj

class ModelMixin:
    def model_dump(self, *_, **__): return _clean(self)
    def model_dump_json(self, *_, **__): return json.dumps(self.model_dump(), sort_keys=True)
...
class SizingRequest(ModelMixin):
    ...
    equity_throttle_schedule: list[EquityThrottleBand] | None = None
    ...
    def __post_init__(self):
        if self.bankroll <= 0: raise ValueError("bankroll must be positive")
        if isinstance(self.edge_estimate, dict): self.edge_estimate = EdgeEstimate(**self.edge_estimate)
        if isinstance(self.liquidity, dict): self.liquidity = Liquidity(**self.liquidity)
        ...
        self.field_confidence = {k: FieldConfidence(v) for k,v in self.field_confidence.items()}

@dataclass
class LayerCap(ModelMixin):
    layer: str
    cap_size: float
    reason: str
    binding: bool = False
...
@dataclass
class Refusal(ModelMixin):
    refused: bool = True
    reasons: list[str] = field(default_factory=list)
    needed_to_proceed: list[str] = field(default_factory=list)
